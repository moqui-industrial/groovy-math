...
class MathDslTest {
    ...
    @Test
    void buildsClosureDeclarationsAgainstEmbeddedSchema() {
        MathMeta mathMeta = MathDsl.math {
            MathModelDef('EmbeddedDef', description: 'Declared without a schema argument')
            // statusId is not-null on the real moqui.math.MathModel: the embedded schema
            // enforces constraints the hand-written test fixture below does not.
            MathModel('EmbeddedModel') {
                mathModelDefId 'EmbeddedDef'
                statusId 'MathModelDraft'
            }
        }.validate()

        assert mathMeta.size() == 2
        assert mathMeta.MathModel.named('EmbeddedModel').get().mathModelDefId == 'EmbeddedDef'
    }
...
'''
    ...
    @Test
    void evaluatesMatrixProductExampleAgainstEmbeddedSchema() {
    ...
    @Test
    void evaluatesProductionPlanExampleAgainstEmbeddedSchema() {
    ...
    @Test
    void evaluatesEnergyDispatchExampleAgainstEmbeddedSchema() {
