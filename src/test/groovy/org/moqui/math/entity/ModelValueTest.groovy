...
class ModelValueTest {
    ...
    @Test
    void requireSeparatesAnUndeclaredFieldFromAnUnsetOne() {
        ModelValue value = new ModelValue(transformationDefinition(), 'DenseProduct')

        // Declared but never set: null is a legitimate answer.
        assert value.require('transformationTypeEnumId') == null
        value.transformationTypeEnumId = 'TtMatrixProduct'
        assert value.require('transformationTypeEnumId') == 'TtMatrixProduct'

        // Undeclared: a bug in the caller, reported here rather than as an NPE later.
        IllegalArgumentException failure = assertThrows(IllegalArgumentException) {
            value.require('dimensionSize')
        }
        assert failure.message.contains('Unknown field')
        assert failure.message.contains('dimensionSize')

        // get() keeps Map semantics, which is exactly why require() is needed.
        assert value.get('dimensionSize') == null
    }
