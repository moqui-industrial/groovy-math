...
import org.moqui.math.libtorch.LibTorchBackend;
...
public final class JaxPanama implements LibTorchBackend {
    ...
    private static SymbolLookup loadSymbols() {
        String explicitPath = System.getProperty("moqui.math.jax.library");
        ...
        try {
            org.moqui.math.nativeutil.NativeLibraryLoader.loadLibrary("groovy_math_jax");
    ...
    /** Input width per live plan handle; see LibTorchPanama for why this is tracked here. */
    ...
    public void executeDirect(long handle, ByteBuffer input, int batchSize, ByteBuffer output) {
        guardPlanInvocation(handle, MemorySegment.ofBuffer(input), MemorySegment.ofBuffer(output), batchSize);
        try {
    ...
    /** Refuses a plan invocation whose segments cannot hold the batch, before it reaches XLA. */
    private void guardPlanInvocation(long handle, MemorySegment input, MemorySegment output, int batchSize) {
        TensorValidator.validatePlanInvocation("jax:" + handle, input, output,
