...
final class ModelValue extends LinkedHashMap<String, Object> {
    ...
    /**
     * Reads a field that must exist in the schema.
     *
     * <p>{@code get} inherits Map semantics and answers null for any key, so a provider that
     * reads a field under a name the schema does not define gets null and fails later as a
     * NullPointerException, or silently takes a default. Both have happened here. This
     * accessor separates the two questions: an undeclared field is a bug in the caller and
     * throws immediately, naming the entity; a declared field that was simply never set
     * still answers null.
     */
    Object require(final String fieldName) {
        if (!definition.fields.containsKey(fieldName)) {
            throw new IllegalArgumentException(
                "Unknown field ${definition.fullName}.${fieldName}; declared fields are ${definition.fields.keySet()}")
        }
        get(fieldName)
    }
